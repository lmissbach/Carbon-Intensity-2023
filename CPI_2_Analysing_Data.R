...
colnames(sum_3.4.1) <- c("Country", rep(c("EQ1","EQ2","EQ3","EQ4","EQ5"),3))
...
# 3.6     Electricity-table ####

data_ely <- read_csv("../0_Data/9_Supplementary Data/Global_Electricity_Production_OWID.csv")

Countries <- c("Bangladesh","Cambodia","India","Indonesia","Iraq","Israel","Maldives","Mongolia","Myanmar","Pakistan", "Philippines","Thailand","Turkey","Vietnam",
               ...
               "Benin","Burkina Faso","Cote d'Ivoire","Ethiopia","Egypt","Ghana","Guinea-Bissau","Kenya","Liberia","Malawi","Mali","Morocco","Mozambique","Niger","Nigeria","Rwanda","Senegal",
               "South Africa", "Togo", "Uganda",
               "Armenia","Norway","United States", "Jordan", "Canada", "United Kingdom", "Georgia", "Austria", "Taiwan","Russia", "Serbia", "Switzerland",
               "Germany", "France", "Netherlands", "Belgium", "Luxembourg", "Czechia", "Poland", "Denmark", "Sweden", "Italy", "Slovakia",
               "Greece", "Hungary", "Croatia", "Portugal", "Spain", "Ireland", "Cyprus", "Bulgaria", "Estonia", "Latvia", "Lithuania", "Finland", "Romania", "Australia")

data_ely.1 <- data_ely %>%
  filter(Entity %in% Countries)%>%
  filter(Year == 2021)

colnames(data_ely.1) <- c("Country", "Code", "Year", "Renewables", "Bioenergy", "Solar", "Wind", "Hydro", "Nuclear", "Oil", "Gas", "Coal")

data_ely.2 <- data_ely.1 %>%
  mutate(Nuclear = ifelse(is.na(Nuclear),0,Nuclear))%>%
  mutate(Gesamt = Renewables + Bioenergy + Solar + Wind + Hydro + Nuclear + Oil + Gas + Coal)%>%
  mutate_at(vars(Renewables:Coal), ~ ./Gesamt)%>%
  mutate_at(vars(Renewables:Coal), ~ ifelse(. != 0, paste0(round(.*100,0),"\\%"), "-"))%>%
  rename("TWh" = Gesamt)%>%
  mutate(TWh = ifelse(TWh < 10, round(TWh, 1), round(TWh, 0)))%>%
  mutate(TWh = prettyNum(TWh))%>%
  select(-Year, -Code)%>%
  mutate(Country = ifelse(Country == "Dominican Republic", "Dominican Rep.", Country))%>%
  select(Country, TWh, Hydro, Wind, Solar, Bioenergy, Renewables, Nuclear, Oil, Gas, Coal)%>%
  rename_at(vars("Hydro":"Coal"), list(~ str_replace(.,"^", "\\\\rotatebox{90}{")))%>%
  rename_at(vars(3:11), list(~str_replace(., "$","}")))

kbl(mutate_all(data_ely.2, linebreak), format = "latex", caption = "Electricity generation in 87 countries (2021)", 
    booktabs = T, align = "l|r|rrrrrrrrr", vline = "", format.args = list(big.mark = ",", scientific = FALSE), linesep = "", escape = FALSE,
    longtable = T, label = "Electricity")%>%
  kable_styling(position = "center", latex_options = c("HOLD_position", "repeat_header"))%>%
  column_spec(1,     width = "3.0 cm")%>%
  column_spec(2:11,  width = "1.6 cm")%>%
  add_header_above(c(" " = 2, "Share of electricity production by source in percent (2021)" = 9))%>%
  footnote(general = "This table provides summary statistics for electricity generation in 88 different countries of our sample. It reports the share of electricity generated by each source in each country in 2021 [\\\\%] as well as the total annual electricity production [TWh]. Source: \\\\textcite{Ember.2023} retrieved through Our World in Data \\\\autocite{Ritchie.2020}.", threeparttable = T, escape = FALSE)%>%
  save_kable(., "2_Tables/Table_Electricity_Generation.tex")

rm(data_ely, data_ely.1, data_ely.2)
...
colnames(data_4.5.1) <- c("Country", "$\\overline{e}_{r}^{1}$", "MAC5", "H1", "H5", "H1A", "H5A", "MAC 1/5", "H 1/5", "H 1/5 A")

kbl(mutate_all(data_4.5.1, linebreak), format = "latex", caption = "Comparing median carbon intensity and horizontal heterogeneity between first and fifth expenditure quintile", 
...
data_6.2.1 <- data_6.2.0 %>%
  ...
  rename(Gender = "Gender HHH", 
         Size = "HH size"
         # Appliance = "Appliance own.", 
         # Electr = "Electricity access",
...
# for Table 1
data_6.2.0_info <- data_6.2.0 %>%
  select(Var_0, Country)%>%
  distinct()%>%
  group_by(Var_0)%>%
  summarise(number = n())%>%
  ungroup()%>%
  filter(Var_0 != "Religiosity")%>%
  arrange(number)
...
pdf("1_Figures/Figures_Appendix/Figure_Silhouette_Clusters_%d.pdf", width = 6.1, height = 3.93, onefile = FALSE)
print(P_6.2.5.3)
print(P_6.2.5.4)
print(P_6.2.5.5)
dev.off()
...
# eval_0 <- read.xlsx("../0_Data/9_Supplementary Data/BRT-Tracking/Tracking_SHAP_Evaluation_VFOLD_2017C_EXP.xlsx")
...
eval_1.3 <- left_join(eval_1.1, eval_1.2)%>%
  left_join(eval_2.1)%>%
  select(Country_long, mean_carbon_intensity, everything(), -Country, -sample)%>%
  select(Country_long, mean_carbon_intensity, ends_with("_wo"), everything())

colnames(eval_1.3) <- c("Country", "Mean", rep(c("MAE", "RMSE", "R\\textsuperscript{2}"),2))
...
eval_1.4 <- left_join(eval_3.1, eval_4.1)%>%
  ...
  arrange(desc(rsq_wo))%>%
  # mutate(test = ifelse(rsq_wo > rsq,1,0))%>%
  mutate(order = 1:n())%>%
...
pdf("1_Figures/Figures_Appendix/Figure_Comparison_Models.pdf", width = 6.1, height = 6.1)
print(P_6.3.1)
dev.off()
...
# Output tables with hyperparameters 

track_0 <- track_0 <- read.xlsx("../0_Data/9_Supplementary Data/BRT-Tracking/Tracking_BRT_2017.xlsx")
track_1 <- track_0 %>%
  group_by(Country)%>%
  dplyr::slice(which.max(number))%>%
  ungroup()%>%
  left_join(select(Country.Set, Country, Country_long))%>%
  select(Country_long, tree_depth_best, learn_rate_best, mtry_best, ends_with("cv_5_train"))%>%
  mutate_at(vars(ends_with("cv_5_train")), ~ round(., 2))%>%
  mutate(learn_rate_best = round(learn_rate_best, 4))%>%
  arrange(Country_long)

colnames(track_1) <- c("Country", "\\texttt{max\\_depth}", "$\\eta$", "\\texttt{mtry}", c("MAE", "RMSE", "R\\textsuperscript{2}"))

kbl(track_1, format = "latex", caption = "Hyperparameters for boosted regression tree models", booktabs = T, align = "l|rcr|ccc", vline = "", format.args = list(big.mark = ",", scientific = FALSE), linesep = "",
    longtable = T, escape = FALSE, label = "C9")%>%
  kable_styling(position = "center", latex_options = c("HOLD_position", "repeat_header"), font_size = 8)%>%
  #column_spec(1, width = "3.15 cm")%>%
...
data_legend <- data.frame(xmin_0   = c(1,1),
                          xmax_0   = c(1,1),
                          ymin_0 = c(0,-1), ymax_0 = c(1,0),
                          Type = c("Richer households more carbon-intensive", "Richer households less carbon-intensive"),
                          y5 = c(-1.5,NA), y25 = c(-0.75,NA), y50 = c(0,NA), y75 = c(0.89,NA), y95 = c(1.59, NA),
                          new_col = c(1,NA),
                          mean = c(0.25, NA))

P_8.Legend <- ggplot(data = data_legend)+
  geom_rect(aes(xmin = xmin_0 - 0.35,
                xmax = xmax_0 + 0.15,
                ymin = ymin_0, ymax = ymax_0, fill = Type), 
            alpha = 0.5, inherit.aes = FALSE)+
  geom_boxplot(aes(ymin = y5, lower = y25, middle = y50, upper = y75, ymax = y95, x = new_col, group = new_col), 
               stat = "identity", position = position_dodge(0.5), outlier.shape = NA, width = 0.4, alpha = 0.85, fill = "grey", size = 0.4)+
  geom_point(aes(y = mean, x = new_col), shape = 23, size = 3 ,fill = "white", stroke = 0.4)+
  theme_void()+
  scale_fill_nejm()+
  ylab(expression(paste("Carbon intensity of consumption [kg", CO[2], "/USD]", sep = "")))+
  coord_flip()+
...
jpeg("1_Figures/Figure 1/Figure_1_2017_Legend_1.jpg", width = 14.5, height = 5, unit = "cm", res = 600)
print(P_8.Legend)
dev.off()
...
for (i in c(1,2,3)){
  ...
  P_8.1.App <- ggplot(filter(data_8.1.4, new_row == i), aes(x = as.character(Income_Group_5)))+
    ...
    geom_boxplot(aes(ymin = y5, lower = y25, middle = y50, upper = y75, ymax = y95), 
                 stat = "identity", position = position_dodge(0.5), outlier.shape = NA, width = 0.5, size = 0.2, alpha = 1, fill = "grey") +
    theme_bw()+
    facet_wrap(. ~ Country_long, scales = "free_x")+
    xlab("Expenditure quintiles")+ ylab(expression(paste("Carbon intensity of consumption [kg", CO[2], "/USD]", sep = "")))+
    geom_point(aes(y = mean), shape = 23, size = 1, fill = "white", stroke = 0.1)+
...
data_8.2.1 <- data_2 %>%
  ...
  summarise(median_carbon_intensity_kg_per_USD_national = wtd.quantile(carbon_intensity_kg_per_USD_national, probs = 0.5, weights = hh_weights),
            q95_carbon_intensity_kg_per_USD_national    = wtd.quantile(carbon_intensity_kg_per_USD_national, probs = 0.95, weights = hh_weights),
            q05_carbon_intensity_kg_per_USD_national    = wtd.quantile(carbon_intensity_kg_per_USD_national, probs = 0.05, weights = hh_weights),
            q10_carbon_intensity_kg_per_USD_national    = wtd.quantile(carbon_intensity_kg_per_USD_national, probs = 0.10, weights = hh_weights),
            q90_carbon_intensity_kg_per_USD_national    = wtd.quantile(carbon_intensity_kg_per_USD_national, probs = 0.90, weights = hh_weights))%>%
  ungroup()%>%
  filter(Income_Group_5 == 1 | Income_Group_5 == 5)%>%
  mutate(dif_q95_q05_carbon_intensity_kg_per_USD_national = q95_carbon_intensity_kg_per_USD_national - q05_carbon_intensity_kg_per_USD_national,
         dif_q90_q10_carbon_intensity_kg_per_USD_national = q90_carbon_intensity_kg_per_USD_national - q10_carbon_intensity_kg_per_USD_national,)%>%
  select(Country, Income_Group_5, dif_q95_q05_carbon_intensity_kg_per_USD_national, dif_q90_q10_carbon_intensity_kg_per_USD_national, median_carbon_intensity_kg_per_USD_national)%>%
...
data_8.2.2 <- data_8.2.1 %>%
  ...
  mutate(horizontal_90_10 = ifelse(dif_90_10_1_5 > 1,1,0))%>%
  mutate(hor_ver          = ifelse(dif_95_05_1_5 > median_1_5,1,0))%>%
  mutate(hor_ver_90_10    = ifelse(dif_90_10_1_5 > median_1_5,1,0))
...
P_8.2_ROB <- ggplot()+
  ...
  geom_label(data = poly_5, aes(label = text, x = x, y = y), alpha = 0.2, size = 2.5)+
  theme_bw()+
  geom_point(data = data_8.2.1, aes(y = median_1_5, x = dif_90_10_1_5, fill = log(value)), shape = 21, colour = "black", size = 3)+
  #geom_text_repel(data = data_7.3, aes(label = Country, y = median_1_5, x = dif_95_05_1_5),
  #                direction = "both", size = 2, max.overlaps = 100)+
  coord_cartesian(xlim = c(0,3.2), ylim = c(0,2.5))+
  scale_fill_viridis_c(breaks = c(6.9,11.1), labels = c("Poorer", "Richer"), name = "Average GDP per capita (log)")+
  ...
  scale_y_continuous(expand = c(0,0), breaks = c(0,1,2))+
  ylab("Vertical distribution coefficient")+
  xlab("Horizontal distribution coefficient (robust)")+
  labs(fill = "")+
  theme(axis.text.y = element_text(size = 7), 
        axis.text.x = element_text(size = 7),
        axis.title  = element_text(size = 7),
...
rm(data_8.2.2, data_8.2.4, data_8.2.4.1, data_8.2.4.2, data_8.2.4.3, data_8.2.4.4, data_8.2.4.5, data_8.2.4.6,
   data_8.2.4.7, data_8.2.4.8, data_8.2.4.9, data_8.2.4.10, data_8.2.4.11, data_8.2.4.12, data_8.2.4.13, data_8.2.4.14, data_8.2.4.15, data_8.2.4.16)
...
eval_3.1 <- eval_3 %>%
  filter(number_ob %in% eval_3.0$number_ob)%>%
  mutate(number = 1:n())%>%
  filter(.metric == "rsq")%>%
  # group_by(Country, number_ob, .metric, fold)%>%
  # filter(number == max(number))%>%
...
data_8.3.2 <- data_8.3.0 %>%
  filter(names == "Vertical distribution")%>%
  group_by(names)%>%
...
P_8.3.2 <- ggplot(data_8.3.2)+
  ...
  scale_x_discrete(labels = c(
    #expression(paste("Horizontal distribution (", widehat(H)[r]^{1} ,")", sep = "")), 
...
P_8.3.4 <- ggplot(data_8.3.5)+
  geom_point(aes(y = cluster, x = names, fill = values), shape = 22, size = 4, stroke = 0.2, colour = "black")+
  theme_bw()+
  #scale_colour_manual(na.value = NA, values = c("black"))+
  scale_fill_gradientn(na.value = NA,
                       colors = c("#0072B5FF", "white","#BC3C29FF", "#631879FF"),
...
pdf("1_Figures/Figure 3/Figure_3_Corrected.pdf", width = 6.1, height = 4.72)
print(P_8.3.5)
dev.off()
...
jpeg("1_Figures/Figure 3/Figure_3_Imputed.jpg", width = 15.5, height = 14.5, unit = "cm", res = 600)
print(P_8.3.4)
dev.off()
...
rm(data_8.3.0, data_8.3.1, data_8.3.2, data_8.3.3, data_8.3.5,
   P_8.3.1, P_8.3.2, P_8.3.3, P_8.3.4, P_8.3.5, L.1)
...
# 8.3.2   Figure 3: Supplementary information ####

data_8.3.2.0 <- read_csv("../0_Data/9_Supplementary Data/BRT-Tracking/Clusters_Normalized_Corrected.csv", show_col_types = FALSE)
data_8.3.2.1 <- read_csv("../0_Data/9_Supplementary Data/WDI/2021_08_17_WDI.csv") %>%
  rename(Country.Name = "Country Name",
         Country.Code = "Country Code",
         Type         = "Series Name")%>%
  select(-'Series Code')%>%
...
data_8.3.2.2 <- left_join(data_8.3.2.0, data_8.3.2.1, by = c("Country" = "Country.Code"))%>%
  select(cluster, Country, value)%>%
  arrange(value)%>%
  mutate(order = 1:n())%>%
...
for(i in c(1,2)){
  ...
  data_8.4.1.1 <- data_8.4.1 %>%
    ...
    filter(names != "dif_95_05_1_5")
  
  P_8.4.1 <- ggplot(data_8.4.1.1)+
    geom_point(aes(y = Country, x = names, fill = values_rescaled), shape = 22, size = size_0, stroke = 0.2)+
    theme_bw()+
    scale_fill_gradientn(na.value = NA,
                         colors = c("#0072B5FF", "white","#BC3C29FF", "#631879FF"),
                         values = scales::rescale(c(0,0.4,0.8,1)),
  ...
  P_8.4.3 <- ggplot(data_8.4.3)+
    geom_point(aes(y = Country, x = names, fill = values_rescaled, colour = help), shape = 22, size = size_0, stroke = 0.2)+
    theme_bw()+
    scale_colour_manual(na.value = NA, values = c("black"))+
    scale_fill_gradientn(na.value = NA,
                         colors = c("#0072B5FF", "white","#BC3C29FF", "#631879FF"),
  ...
  P_8.4.4 <- ggplot(data_8.4.4)+
    geom_point(aes(y = Country, x = .metric, fill = R2), shape = 22, size = size_0, stroke = 0.2, colour = "black")+
    theme_bw()+
    #scale_colour_manual(na.value = NA, values = c("black"))+
    scale_fill_gradientn(na.value = NA,
                         colors = c("#0072B5FF", "white","#BC3C29FF", "#631879FF"),
  ...
  pdf(sprintf("1_Figures/Figure 4/Figure_4_Uncorrected_%s.pdf",i), width = 6.1, height = 7.09, onefile = FALSE)
  print(P_8.4.5)
  dev.off()
}
...
# Carbon intensity of consumption - scaling cannot do harm
  ...
  pdf(sprintf("1_Figures/Figure 4/Figure_4_Corrected_%s.pdf",i), width = 6.1, height = 7.09, onefile = FALSE)
  print(P_8.4.5)
  dev.off()
}
...
for(i in c(1,2)){
  ...
  pdf(sprintf("1_Figures/Figure 4/Figure_4_Corrected_Imputed_%s.pdf",i), width = 6.1, height = 7.09, onefile = FALSE)
  print(P_8.4.5)
  dev.off()
}
...
kbl(data_8.4.1, format = "latex", caption = "Feature importance across countries by cluster - imputed", booktabs = T, 
    vline = "", format.args = list(big.mark = ",", scientific = FALSE), linesep = "",
    longtable = T, label = "A10_Imputed")%>%
  kable_styling(position = "center", latex_options = c("HOLD_position", "repeat_header"), font_size = 9)%>%
  row_spec(0, angle = 90)%>%
  row_spec(1:88, font_size = 8)%>%
  column_spec(1, width = "0.35 cm")%>%
...
eval_8.5 <- read.xlsx("../0_Data/9_Supplementary Data/BRT-Tracking/Tracking_SHAP_Evaluation_VFOLD_2017C.xlsx")
...
data_8.5.1 <- read.xlsx("../0_Data/9_Supplementary Data/BRT-Tracking/Tracking_SHAP_Classification_VFOLD_2017C.xlsx")%>%
...
data_8.5.2.B <- read.xlsx("../0_Data/9_Supplementary Data/BRT-Tracking/Tracking_SHAP_Detail_VFOLD_2017C.xlsx")%>%
  filter(number_ob %in% eval_8.5.1$number_ob)%>%
  mutate(lag_country = ifelse(Country != lag(Country), 1:n(),NA))%>%
  fill(lag_country)%>%
  mutate(lag_country = ifelse(is.na(lag_country),1,lag_country))%>%
...
for (i in c(data_8.5.2.C$Country)){
    ...
    data_8.5.2.0 <- read_rds(sprintf("../0_Data/9_Supplementary Data/BRT-Tracking/SHAP-Values en detail/2017_C/SHAP_wide_%s.rds",i))
...
list_all <- list()
 ...
 for (i in c(Country.Set$Country)){
  ...
  pdf(sprintf("1_Figures/Figure 5b/Figure_5b_%s.pdf", i), width = 11.81, height = 3.94)
  print(P_8.5.2.5)
  dev.off()
...
Country.Set.8.6.1 <- Country.Set %>%
  mutate(Group_1 = c(rep("A",30), rep("B",30), rep("C",28)))
...
data_8.6.2 <- read_csv("../0_Data/9_Supplementary Data/BRT-Tracking/Clusters_Normalized_Corrected_C.csv", show_col_types = FALSE)%>%
  ...
  select(cluster, Country, order, best_fit, largest_country, everything())%>%
  left_join(select(Country.Set, Country, Country_long))%>%
  select(cluster, Country_long, everything())

data_8.6.2.1 <- data_8.6.2 %>%
  select(Country_long, cluster, Country)

data_8.6.2.2 <- data_8.6.2 %>%
  select(Country_long, "Appliance own.":"Spatial")%>%
  pivot_longer(c("Appliance own.":"Spatial"), values_to = "imp", names_to = "feature")%>%
...
world_map <- map_data("world")%>%
  mutate(region = ifelse(region == "Myanmar", "Myanmar (Burma)",
                         ifelse(region == "Ivory Coast", "Côte d’Ivoire",
                                ifelse(region == "Czech Republic", "Czechia",
                                       ifelse(region == "UK", "United Kingdom",
                                              ifelse(region == "USA", "United States", region))))))%>%
  left_join(data_8.6.2.1, by = c("region" = "Country_long"))%>%
  filter(region != "Antarctica")%>%
  left_join(data_8.6.2.2, by = c("region" = "Country_long"))%>%
  left_join(data_8.6.2.3)
...
P_8.6.2.2 <- ggplot(world_map, aes(x = long, y = lat, group = group))+
  geom_polygon(fill = "lightgrey", size = 0.2)+
  geom_polygon(aes(fill = rsq), colour = "black", size = 0.2)+
  scale_fill_material(palette = "deep-purple", na.value = "lightgrey", name = expression(paste("Goodness of fit ", (R^2))),
                      limits = c(0,0.7001), labels = scales::percent_format(), breaks = c(0,0.2,0.4,0.6))+
  theme_bw()+
  theme(legend.position = "bottom",
        axis.text  = element_blank(),
        axis.ticks = element_blank(),
...
# Figure on most important feature

P_8.6.2.3 <- ggplot(world_map, aes(x = long, y = lat, group = group)) +
  geom_polygon(fill = "lightgrey", size = 0.2)+
  geom_polygon(aes(fill = feature), colour = "black", size = 0.2)+
  scale_fill_brewer(na.value="grey", na.translate = FALSE, palette = "Spectral")+
  theme_bw()+
  labs(fill = "Single most important feature")+
...
jpeg("1_Figures/Figures_Appendix/Figure_Maps_1.jpg", width = 15.5, height = 9, unit = "cm", res = 600)
print(P_8.6.2.1)
dev.off()

jpeg("1_Figures/Figures_Appendix/Figure_Maps_2.jpg", width = 15.5, height = 10, unit = "cm", res = 600)
print(P_8.6.2.2)
dev.off()

jpeg("1_Figures/Figures_Appendix/Figure_Maps_3.jpg", width = 15.5, height = 11, unit = "cm", res = 600)
print(P_8.6.2.3)
dev.off()

pdf("1_Figures/Figures_Appendix/Figure_Maps_1.pdf", width = 6.1, height = 3.54)
print(P_8.6.2.1)
dev.off()

pdf("1_Figures/Figures_Appendix/Figure_Maps_2.pdf", width = 6.1, height = 3.94)
print(P_8.6.2.2)
dev.off()
...
data_8.7 <- data_2 %>%
  mutate(share_s_cooking_fuels   = burden_s_cooking_fuels/burden_CO2_national,
         share_s_transport_fuels = burden_s_transport_fuels/burden_CO2_national,
         share_s_goods           = burden_s_Goods/burden_CO2_national,
         share_s_services        = burden_s_Services/burden_CO2_national,
         share_s_food            = burden_s_Food/burden_CO2_national,
         share_s_Electricity     = burden_s_Electricity/burden_CO2_national,
         share_s_other_energy    = burden_s_other_energy/burden_CO2_national)%>%
  mutate(share_s_cooking_and_energy = share_s_cooking_fuels + share_s_other_energy)%>%
  select(-share_s_cooking_fuels, -share_s_other_energy)%>%
  select(Country, hh_weights, starts_with("share_s"), - share_services)%>%
  pivot_longer(starts_with("share_s"), names_to = "type", values_to = "share")%>%
  group_by(Country, type)%>%
  summarise(mean = wtd.mean(share, hh_weights),

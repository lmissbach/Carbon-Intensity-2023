...
kbl(mutate_all(data_8.2.4.10, linebreak), format = "latex", 
  ...
  footnote(general = "This table compares vertical and horizontal distribution coefficients for different policy instruments across all countries. 
           ...
           Column '$\\\\widehat{V}^{1} \\\\uparrow$' displays the number of countries in which $\\\\hat{V}^{1}$ increases in comparison to national climate policy, i.e., in which poorer households would consume more carbon-intensively compared to richer households and to the 'national climate policy'-scenario.
           Column '$\\\\widehat{V}^{1} \\\\downarrow$' displays the number of countries in which $\\\\hat{V}^{1}$ decreases in comparison to national climate policy, i.e., in which poorer households would consume less carbon-intensively compared to richer households and to the 'national climate policy'-scenario.
           ...
           Column '$\\\\widehat{H}^{1} \\\\uparrow$' displays the number of countries in which $\\\\hat{H}^{1}$ increases in comparison to national climate policy, i.e., in which heterogeneity among poorer households compared to richer households would increase in comparison to the 'national climate policy'-scenario.
           Column '$\\\\widehat{H}^{1} \\\\downarrow$' displays the number of countries in which $\\\\hat{H}^{1}$ decreases in comparison to national climate policy, i.e., in which heterogeneity among poorer households compared to richer households would decrease in comparison to the 'national climate policy'-scenario.",

...
\begin{itemize}
    ...
    \item Stress that some novel data was acquired in the meantime (India, Germany, France, Romania, Spain), some previous analyses have been refined (Mongolia, ...)
\end{itemize}
\end{response}
...
This analysis corroborates that heterogeneity between expenditure quintiles contributes -- on average -- only 2\% to overall heterogeneity. As a robustness check, we also include this analysis with expenditure deciles -- addressing your substantiated \nameref{comment_R1_1} --, where we find that heterogeneity between expenditure quintiles contributes X\% to overall heterogeneity. Furthermore, we analyze a variation of the Theil index (GE(1)) that accounts more explicitly for long right-tail distributions. Both checks corroborate our core argument that between expenditure group heterogeneity is insufficient to analyze overall heterogeneity.
...
\begin{response}
    Expenditures on public transport fits best to \textit{land transport and transport via pipelines} (\textit{otp}), which is consistent with substantially lower carbon intensities across countries in comparison to transport fuels (\textit{p\_c}).
...
\begin{response}
    Thank you for this suggestion. The horizontal distribution coefficient indicates whether horizontal heterogeneity is larger among poorer and richer households and serves mainly a descriptive purpose. Even though two countries appear in the same of four different cells in Figure 2, it does not necessarily follow that effective compensation would like similar in both places. 
    
    To better describe heterogeneity between and within expenditure groups -- and in response to two comments from Reviewer 1 -- we have decided to include a more precise measure based on the Theil index. This novel analysis corroborates that heterogeneity between expenditure quintiles is generally insufficient for explaining overall heterogeneity -- an argument that is also supported by Figure 2, but not yet sufficiently clear.

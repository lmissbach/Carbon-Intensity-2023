...
Admittedly, policymakers need not only to consider different options for compensation, but may also consider different types of regulation. % Add here section on different policy instruments.

\begin{itemize}
\item Include paragraph on different policies (see below).
% Discuss role of findings for design of different policies
\item Notably, all instruments entail distributional impacts, one form or the other, even though price-based policies may be particularly salient. Discuss role of findings for design of different policies (standards, subsidies, brief ?) Non-pricing interventions can be helpful, in case they change preferences or address technological path-dependencies.
...
\item Measurement error concern
\item Clustering subject to uncertainty, but evident that some compensation tools work better in some countries and contexts than in others.
% Discuss value chain of climate policy implementation. Public support of which groups matter for people with political power? What future research might look like. Welfare function of policymakers or winning coalition not clear.
\item It is not only economists who conduct welfare analysis, but also those that engage with the political economy that have been interested in distributional impacts of welfare-enhancing policy proposals. While this study provides a comprehensive assessment about how costs distribute among the population, it is less clear how the distribution of costs is \textit{perceived} by the public. Often, it is argued that people prefer progressive outcomes out of justice considerations, but large horizontal heterogeneity, subjective beliefs \autocite{Douenne.2022} and scattered perceptions of fairness \autocite{MaestreAndres.2019,Povitkina.2021} cast doubt on this assumption. Future research could contribute to better understand how the distribution of policy costs shapes the public acceptability of climate policy. Similarly, some policy instruments and complementary compensation mechanisms may be more acceptable to the population than others, but research providing theory and empirical evidence remains sparse, at least in comparison to the literature quantifying distributional impacts. 
\item Such analysis has a mainly informative character and can only provide the foundation for more comprehensive analysis. Nevertheless, Information may be used by policymakers to draw conclusions about the distribution of climate policy and identify \textit{need}, if anything, for compensation.
...
Nevertheless, whether policy instrument can help to raise revenues should be important part of policy evaluation. More sectorally focussed regulation, less aggregate cost-effectiveness and less revenues.

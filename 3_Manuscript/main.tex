...
\paragraph{Boosted regression trees} Fitting boosted regression trees (BRT) (REF ELITH) is a supervised machine learning method allowing for detection of non-linear relationships and interaction effects between an outcome and many predictor variables (features). As an extension to regression trees, the BRT algorithm (XGBoost) fits many regression trees giving higher weights to observations with larger predicting errors. This leads to large predictive power, also if compared to more frequently used random forest algorithms. Nevertheless, BRT are usually more computationally intensive and  more difficult to interpret. % Why BRT is helpful here.

Deploying BRT serves the purpose of our analysis, because it is a priori ambiguous which household characteristics to include in a model. BRT-modelling is well suited to help identifying important predictors and potentially non-linear effect sizes.
...
K-means clustering helps to analyze clusters of observations which are most similar in many variables within each cluster and least similar in many variables across clusters (REF MacQueen 1967). We inspect the optimal number of clusters with the help of average silhouette widths for each cluster. The silhouette width represents the average distance of each observation to all other observations within its cluster and the average distance to observations from the nearest neighbouring cluster. Silhouettes closer to $1$ indicate a good fit of an observation to its cluster and silhouettes closer to $-1$ indicate a poor fit. The average silhouette width expresses how well each observations fits to each cluster. Our approach yields $k = 14$ to be the number of clusters maximizing average silhouette width\footnote{See Appendix X for visualization.}.
...
Figure \ref{fig:fig_3} shows the relative importance of different features across country-clusters. %What are important criteria to assign countries to different clusters?
...
\begin{figure}[ht!]
    ...
    \label{fig:fig_3}
    \begin{subcaption}
    This figure shows the average importance of features (in normalized absolute average SHAP-values) across all countries from each cluster A to N. Colours express the average importance of features in a cluster in comparison to other clusters. Blue (red) colours indicate that a feature is relatively less (more) important on average in a cluster compared to all other clusters. 'Sociodemographic' comprises normalized absolute average SHAP values for features such as ethnicity, nationality, religion or language. 
    For horizontal inequality, blue (red) colours indicate a lower (higher) heterogeneity within the first income quintile compared to the fifth income quintile. For vertical inequality, blue (red) colours indicate lower (higher) median carbon intensity among the first income quintile compared to the fifth income quintile. For average $CO_{2}$-intensity, blue (red) colours indicate a lower (higher) average carbon intensity across clusters. We assign countries to 14 clusters performing k-means clustering. We also show all values in Table REF.
    \end{subcaption}
\end{figure}
...
\begin{figure}[ht!]
    ...
    \label{fig:fig_4_1}
    \begin{subcaption}
    This figure shows the importance of features (in normalized average absolute SHAP-values) for each country, grouped by country clusters. Blue (red) colours indicate that a feature is relatively less (more) important in a country compared to all other countries. 'Sociodemographic' comprises normalized absolute SHAP-values for features such as ethnicity, nationality, religion or language.
    For horizontal inequality, blue (red) colours indicate a lower (higher) heterogeneity within the first income quintile compared to the fifth income quintile. For vertical inequality, blue (red) colours indicate lower (higher) median carbon intensity among the first income quintile compared to the fifth income quintile. For average $CO_{2}$-intensity, blue (red) colours indicate a lower (higher) average carbon intensity across all countries.
    We assign countries to 14 clusters performing k-means clustering based on scaled values across all features. We also show all values in Table REF.
    \end{subcaption}
\end{figure}
...
\begin{figure}[ht!]
    ...
    \label{fig:fig_5_1}
    \begin{subcaption}
    This figure shows SHAP-values for predicting carbon intensity over feature values for one representative country in each country-cluster in rows. All values are from the testing set. The bar chart displays normalized average absolute SHAP-values for all features. Features with less than 3\% of normalized SHAP-values are subsumed as "Other features (Sum)". Charts show SHAP-values over total household expenditures for all countries and for the three most important features in each country besides total household expenditures. Colours represent household expenditures with blue (red) colours indicating lower (higher) household expenditures. See also Appendix REF to REF.
    \end{subcaption}
\end{figure}

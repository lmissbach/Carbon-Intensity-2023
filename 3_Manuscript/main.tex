...
\begin{itemize}
    ...
    \item Governments can use exogenously given budget (possibly including proceeds from carbon taxation) to finance transfers. Strict budget constraint including administrative costs not necessary for now, but model could easily extended to calculate how many resources should be spent to increase targeting precision, assuming that reducing heterogeneity in costs is warranted. Can be $A = \sum_{X'} (1 + \alpha_{X'}) T_{i,X'} + T_{i,\varepsilon}$ and A can equal zero. $\alpha_{X'}$ represent the fraction of administrative costs for each transfer T\textsubscript{X'}.

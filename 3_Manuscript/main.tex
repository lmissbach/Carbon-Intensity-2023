...
For a long time, this basic tenet of economic theory has guided our thinking about the distributional effects of climate policy. In practice, however, governments face important information constraints. Ex ante, it is often unclear how the costs of climate policy will be distributed across households, whether the use of of existing compensation policies will be sufficient to address these costs, and what new instruments would be needed to achieve any distribution of costs that is politically desirable. In particular, large horizontal heterogeneity \autocite{Fischer.2019,Hansel.2022} can reduce the effectiveness of uniform lump-sum transfers, which are oft-proposed for their administrative simplicity. Designing effective compensation policies, i.e., minimizing targeting errors, is thus important to advance public support for climate policy \autocite{MaestreAndres.2019,Dechezlepretre.2025} and to ease fiscal pressure. Yet, our understanding of which compensation policies would be effective in achieving which distribution of costs \textit{post}-compensation is crude.
...
We proceed as follows: In Section \ref{sec:literature}, we develop a theoretical framework describing the distributional impacts of climate policy and their relevance for governments to design complementary compensation policies. In Section \ref{sec:data_methods}, we describe our modeling approach and analytical methods. In Section \ref{sec:results}, we analyze the vertical and horizontal distributional effects of climate policy, present the relative importance of household characteristics for predicting carbon intensity at the country level, and cluster countries when household characteristics are similarly important. Finally, we discuss our findings in light of ongoing debates about how to avoid or address unintended distributional impacts of climate policy in Section \ref{sec:discussion} before concluding in Section \ref{sec:conclusion}.  
...
% Table TBD: Share of consumption items on carbon intensity of consumption
\input{2_Tables/Table_Summary_A8_Share_Consumptions}
\clearpage

% Table TBD: Theil index and robustness checks
